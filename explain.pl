...
my $handler
  = DBIx::Handler->new( "DBI:mysql:database=information_schema:127.0.0.1",
  $db_user, $db_passwd, $db_opt );

my $db;
my $long_query_time
  = $handler->dbh->selectrow_array(
...
# そのままMessagePackで放出する
  ...
  my $data   = $decode;
  my $result = eval {
    $handler->dbh->selectall_arrayref( "explain $select", +{ Slice => {} } );
  };
